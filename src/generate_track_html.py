...
<head>
    ...
    <style>
        ...
        .legend { text-align: left; line-height: 18px; color: #555; max-height: 500px; overflow-y: auto; transition: max-height 0.3s ease-in-out; }
        .legend i { width: 18px; height: 18px; float: left; margin-right: 8px; opacity: 0.7; }
        .legend-toggle { cursor: pointer; font-weight: bold; margin-bottom: 5px; user-select: none; }
        .legend-toggle:after { content: ' ▼'; font-size: 12px; }
        .legend-toggle.collapsed:after { content: ' ►'; }
        .legend-content { display: block; }
        .legend-content.collapsed { display: none; }
        .legend-section { margin-bottom: 10px; }
        @media (max-width: 768px) {
            .legend { max-width: 80%; max-height: 300px; }
        }
...
""")
    ...
    legend.onAdd = function(map) {
        ...
        // Add toggle button and content container
        legendContent = '<div class="legend-toggle">Legend</div><div class="legend-content">';
        
        // Add segment type legend
        legendContent += '<h4>Segment Types</h4>';
        
        // Add legend for segment types
        legendContent += '<div><i style="background:rgb(255,0,0)"></i> Turn (even)</div>';
        legendContent += '<div><i style="background:rgb(255,100,0)"></i> Turn (odd)</div>';
        
        // Add speed category legend
        legendContent += '<hr><h4>Speed Categories (Straight Segments)</h4>';
        
        // Add legend for speed categories with paired colors
        legendContent += '<div><i style="background:rgb(80,200,80)"></i> Slow (even) < 4 knots</div>';
        legendContent += '<div><i style="background:rgb(220,220,60)"></i> Slow (odd) < 4 knots</div>';
        legendContent += '<div><i style="background:rgb(60,80,220)"></i> Medium (even) 4-6 knots</div>';
        legendContent += '<div><i style="background:rgb(180,60,220)"></i> Medium (odd) 4-6 knots</div>';
        legendContent += '<div><i style="background:rgb(220,60,60)"></i> Fast (even) > 6 knots</div>';
        legendContent += '<div><i style="background:rgb(240,140,40)"></i> Fast (odd) > 6 knots</div>';
        
        // Add trend line legend
        legendContent += '<hr><h4>Trend Lines</h4>';
        legendContent += '<div><i style="background:black; height:2px; margin-top:8px; opacity:0.7"></i> Overall segment</div>';
        legendContent += '<div><i style="background:blue; height:2px; margin-top:8px; opacity:0.7"></i> First 10 seconds</div>';
        legendContent += '<div><i style="background:red; height:2px; margin-top:8px; opacity:0.7"></i> Last 10 seconds</div>';
        
        // Add tack points legend
        legendContent += '<hr><h4>Tack Points</h4>';
        legendContent += '<div><i style="background:gray; width:6px; height:6px; border-radius:50%; display:inline-block"></i> Potential Tack</div>';
        legendContent += '<div><i style="background:yellow; width:8px; height:8px; border-radius:50%; display:inline-block"></i> Valid Tack (>10s)</div>';
        legendContent += '<div><i style="background:green; width:10px; height:10px; border-radius:50%; display:inline-block"></i> Used for Wind Direction</div>';
        
        // Add course markers legend if present
        if (trackData.buoys && trackData.buoys.length > 0) {
            legendContent += '<hr><h4>Course Markers</h4>';
            ...
            if (hasMarkers) {
                legendContent += '<div><i style="background:red; width:10px; height:10px; border-radius:50%; border:1px solid white; display:inline-block; color:white; text-align:center; font-weight:bold; font-size:8px;">B</i> Buoy</div>';
            }
            
            if (hasLines) {
                legendContent += '<div><i style="background:red; width:15px; height:3px; display:inline-block;"></i> Line</div>';
            }
            
            if (hasPolygons) {
                legendContent += '<div><i style="background:rgba(0,0,255,0.1); border:2px solid blue; width:15px; height:10px; display:inline-block;"></i> Area</div>';
            }
        }
        
        // Add segment count info
        legendContent += '<hr><h4>Segment Stats</h4>';
        ...
        legendContent += '<div><b>By Type:</b></div>';
        legendContent += '<div>Turn: ' + typeCounts.turn + '</div>';
        legendContent += '<div>Straight: ' + typeCounts.straight + '</div>';
        ...
        legendContent += '<div><b>By Speed:</b></div>';
        legendContent += '<div>Slow: ' + speedCounts.slow + '</div>';
        legendContent += '<div>Medium: ' + speedCounts.medium + '</div>';
        legendContent += '<div>Fast: ' + speedCounts.fast + '</div>';
        legendContent += '<b>Total: ' + trackData.length + '</b>';
        ...
        if (trackData.length > 0 && trackData[0].metrics.wind_direction !== undefined) {
            ...
            legendContent += '<hr><h4>Tack Distribution</h4>';
            legendContent += '<div>Port: ' + tackCounts.port + '</div>';
            legendContent += '<div>Starboard: ' + tackCounts.starboard + '</div>';
            if (tackCounts.mixed > 0) {
                legendContent += '<div>Mixed: ' + tackCounts.mixed + '</div>';
            }
            ...
            legendContent += '<hr><h4>Points of Sail</h4>';
            for (const [sail, count] of Object.entries(pointOfSailCounts)) {
                if (count > 0) {
                    legendContent += `<div>${sail}: ${count}</div>`;
                }
            }
        }
        
        // Close the legend-content div
        legendContent += '</div>';
        ...
        // Also use Leaflet's event handling for compatibility
        L.DomEvent.on(toggle, 'click', function(e) {
            L.DomEvent.stopPropagation(e);
            L.DomEvent.preventDefault(e);
        });
        
        // Prevent map click events when interacting with the legend
